...
let giveRandomMeme = async () => {
    try {
            // database query to get random meme
            // gen a random no
            ...
            let randomImage = Math.floor(Math.random() * cnt);
            console.log(randomImage, " randomImage")
            imageName = "image" + randomImage;
            const selectTableQuery = `
                SELECT * FROM images WHERE IMAGENAME='${imageName}'
            `
            const val = await connection.executeQuery(selectTableQuery);
            const resp = val[0]["IMAGEDATA"];
